...
function setErrorMessage(error) {
  let message = messages[error.message] || messages['default_message'] || error.message;

  if (error.hasOwnProperty('replace')) {
    if (typeof error.replace === 'string') {
      message = message.replace('%s', error.replace);
    } else {
      error.replace.forEach((string) => {
